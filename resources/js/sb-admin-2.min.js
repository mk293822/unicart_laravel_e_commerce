...
function urlWorkingTogether() {
    ...
    if (sort) {
        ...
        const u = userSort.querySelector('select[name="u"]')?.value;
        const directionUser = userSort.querySelector('select[name="directionUser"]')?.value;
        if (u) {
            params.set('u', u);
        }
        if (directionUser) {
            params.set('directionUser', directionUser);
        }
    }
...
if (userSort) {
    userSort.addEventListener('change', (e) => {
        e.preventDefault();
        urlWorkingTogether();
    });
}
